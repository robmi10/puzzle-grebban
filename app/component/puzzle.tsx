            ...
            puzzleList.map((column, i) => {
                return (
                    <div key={i} className='relative z-0'>
                        <div onClick={() => {
                            handlePosition(i)
                        }} className={twMerge('border z-10 hover:bg-blue-600 transition-colors duration-200 absolute h-full w-full cursor-pointer bg-blue-400 rounded-xl flex justify-center items-center font-bold text-2xl',
                            !column?.value && 'hidden z-0'
                        )}>
                            {column.value}
                        </div>
...
const Puzzle = () => {
    ...
    return (
        <div className='w-full h-full flex justify-center items-center flex-col space-y-12'>
            <div ref={containerRef} className='w-full h-2/4 border relative border-black rounded-md md:w-2/4 md:h-2/4 grid grid-cols-5'>
                <Board puzzleList={puzzleList} handlePosition={handlePosition} />
            </div >
            <button className='border border-black w-56 h-24 rounded-full hover:bg-gray-800 transition-colors duration-200 ease-in-out bg-gray-600 text-white text-2xl' onClick={() => { shuffleList() }}>Slumpa</button>
        </div>
    )
}
